...
public class TelecomUtils {
    ...
    public static final PhoneAccountHandle getDefaultOutgoingPhoneAccount(Activity activity, TelecomManager telecomManager, String str) {
        if (telecomManager != null && CallerHelper.isMethodAvailable("android.telecom.TelecomManager", "getDefaultOutgoingPhoneAccount", String.class) && activity.checkPermission("android.permission.READ_PHONE_STATE", Process.myUid(), Process.myPid()) == PackageManager.PERMISSION_GRANTED) {
    ...
    private static final TelecomManager getTelecomManager(Context context) {
        return (TelecomManager) (context == null ? null : context.getSystemService(Context.TELECOM_SERVICE));
    }
